...
char	documentRoot[] = "/home/yb19/week12_yb";
...
main(int argc, char *argv[]) {
	...
	web_log(LOG, "STATUS2","Server is listening", port);
...
do_web(void* arg)
{
	...
	if(param != NULL) {
 	...
 	if(strstr(param, "LED_ON"))
        web_log(LOG, "LED", "LED 켜짐", 0);

	if(strstr(param, "LED_OFF"))
        web_log(LOG, "LED", "LED 꺼짐", 0);

    	if(strstr(param, "BUZ_ON"))
        web_log(LOG, "BUZ", "부저 켜짐", 0);

    	if(strstr(param, "BUZ_OFF"))
        web_log(LOG, "BUZ", "부저 꺼짐", 0);
}
